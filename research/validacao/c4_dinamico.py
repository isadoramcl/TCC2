...
Uso: python c4_dinamico.py PARTE/N"""
import sys,time
...
def job(a):
    ...
    op=yaml.safe_load((ROOT/'config/mvp.yaml').read_text())['opcoes']; op.update(instrumentar_tarefas=False,limite_horizonte_fator=16)
...
if __name__=='__main__':
    ...
    d.to_csv(ROOT/f"outputs/diagnosticos/20260921_c4_dinamico/c4_dinamico_p{i}de{n}.csv",index=False); print(len(d),'execucoes',round(time.time()-t0),'s')
