...
def consolidar(base):
    ...
    controle=num[num.configuracao=='legado'][['metrica','numero','DEPOIS']].rename(columns={'DEPOIS':'CONTROLE_mesmo_ambiente'})
    num=num.merge(controle,on=['metrica','numero'],validate='many_to_one')
    num['delta_vs_controle']=num.DEPOIS-num.CONTROLE_mesmo_ambiente
    num.to_csv(out/'antes_depois_numeros_experimento.csv',index=False)
    ...
    for modelo in ['legado','mvp']:
        ...
        for m,g in d.groupby('metrica'):
            ...
            for braco,terms in {'so_tau_inicial':['G','N','GN'],'so_tau_min':['B'],'so_p_reporte':['C'],'so_p_deteccao':['D'],'so_portao':['G'],'so_rede':['N'],'centralizada':[],'adaptativa':g.termo.tolist(),'so_par_tau':[t for t in g.termo if set(t)<=set('GNB')],'sem_par_tau':['C','D','CD']}.items():
    ...
    numeros_publicados(base,out,alt,b9)
    ...
    pd.DataFrame([dict(arquivo=str(f.relative_to(ROOT)) if f.is_relative_to(ROOT) else str(f),sha256=hashlib.sha256(f.read_bytes()).hexdigest()) for f in sorted(set(files))]).to_csv(out/'hashes_evidencias.csv',index=False)
    (out/'controles.json').write_text(json.dumps(dict(max_diferenca_legado=float(maxdiff),n_legado=384,n_diagonais=3072),indent=2)+'\n')



def numeros_publicados(base,out,alt,b9):
    """Mapeia cada coluna numérica publicada da B9/fatorial ao mesmo estimando.

    Fatorial antigo A é a restrição diagonal G=N. O 2^5 completo é reportado
    em outra tabela, para não fingir que 31 termos são os mesmos 15 termos.
    """
    registros=[]
    def registrar(fonte,modelo,chaves,before,after):
        for k,value in after.items():
            if k in before:
                registros.append(dict(fonte=fonte,modelo=modelo,**chaves,numero=k,ANTES=before[k],DEPOIS=value,delta=value-before[k]))
    def stat(x,y):
        r=resumo_estat(x,y);delta=y-x
        return dict(media_centralizada=x.mean(),media_adaptativa=y.mean(),dif_absoluta=delta.mean(),dif_relativa_pct=r['variacao_percentual'],p_valor=r['p_instancia'],d_cohen_pareado=r['d_instancia'],prop_instancias_com_dif=float(np.mean(delta!=0)),n_instancias=len(x))
    oldp=pd.read_csv(T/'modelo_07_ablacao_parametros.csv')
    oldm=pd.read_csv(T/'modelo_07_ablacao_mecanismo.csv')
    oldf=pd.read_csv(T/'modelo_10_fatorial_decomposicao.csv')
    letras='GNBCD';cells=[''.join(map(str,c)) for c in itertools.product([0,1],repeat=5)]
    terms=['intercepto']+[''.join(t) for k in range(1,6) for t in itertools.combinations(letras,k)]
    M=np.array([[1 if t=='intercepto' else np.prod([int(c[letras.index(l)]) for l in t]) for t in terms] for c in cells]);inv=np.linalg.inv(M)
    mapa={'centralizada':'00000','adaptativa':'11111','so_tau_inicial':'11000','so_tau_min':'00100','so_p_reporte':'00010','so_p_deteccao':'00001','so_par_tau':'11100','sem_par_tau':'00011'}
    for modelo in ['legado','mvp']:
        raw=pd.read_csv(base/f'canais_{modelo}/bruto.csv',dtype={'celula':str});raw.celula=raw.celula.str.zfill(5)
        for met in oldf.metrica.unique():
            tab=raw.groupby(['celula','arquivo'])[met].mean().unstack('arquivo').reindex(cells)
            coef=inv@tab.values; total=tab.loc['11111'].values-tab.loc['00000'].values
            coef4={}
            for t in oldf[oldf.metrica==met].termo:
                if t.startswith('SOMA'):continue
                ts=['G'+t[1:],'N'+t[1:],'GN'+t[1:]] if t.startswith('A') else [t]
                coef4[t]=sum(coef[terms.index(x)] for x in ts)
            coef4['SOMA_principais']=sum(v for k,v in coef4.items() if len(k)==1)
            coef4['SOMA_interacoes']=sum(v for k,v in coef4.items() if len(k)>1 and not k.startswith('SOMA'))
            for t,values in coef4.items():
                before=oldf[(oldf.metrica==met)&(oldf.termo==t)].iloc[0]
                after=dict(parcela_media=values.mean(),pct_do_contraste=100*values.mean()/total.mean() if abs(total.mean())>1e-12 else np.nan,p_valor=float(stats.wilcoxon(values).pvalue) if not np.allclose(values,0) and not t.startswith('SOMA') else np.nan,n_instancias=len(values))
                registrar('modelo_10_fatorial_decomposicao.csv',modelo,dict(metrica=met,termo=t),before,after)
            for r in oldp[oldp.metrica==met].itertuples():
                x=tab.loc['00000'].values;y=tab.loc[mapa[r.braco]].values
                after=stat(x,y);after.update(media_referencia_centralizada=x.mean(),media_braco=y.mean(),efeito_total_adaptativa=total.mean(),fracao_do_efeito_total_pct=100*(y-x).mean()/total.mean() if total.mean() else np.nan)
                registrar('modelo_07_ablacao_parametros.csv',modelo,dict(metrica=met,termo=r.braco),r._asdict(),after)
    for r in oldm.itertuples():
        g=b9[b9.configuracao==r.configuracao];p=g.groupby(['arquivo','cenario'])[r.metrica].mean().unstack()
        nom=b9[b9.configuracao=='nominal'].groupby(['arquivo','cenario'])[r.metrica].mean().unstack();total=(nom.adaptativa-nom.centralizada).mean()
        after=stat(p.centralizada.values,p.adaptativa.values);after.update(efeito_nominal_absoluto=total,efeito_restante_pct_do_nominal=100*after['dif_absoluta']/total if total else np.nan)
        registrar('modelo_07_ablacao_mecanismo.csv','mvp',dict(metrica=r.metrica,termo=r.configuracao),r._asdict(),after)
    # P auto depende da implementação e de empates; controle calcula os mesmos
    # números no ambiente atual antes de atribuir qualquer delta ao MVP.
    oldraw=pd.read_csv(T/'modelo_07_ablacao_bruto.csv')
    for r in oldm.itertuples():
        bc='centralizada'+('' if r.configuracao=='nominal' else '__'+r.configuracao)
        ba='adaptativa'+('' if r.configuracao=='nominal' else '__'+r.configuracao)
        x=oldraw[oldraw.braco==bc].groupby('arquivo')[r.metrica].mean().sort_index().values
        y=oldraw[oldraw.braco==ba].groupby('arquivo')[r.metrica].mean().sort_index().values
        nc=oldraw[oldraw.braco=='centralizada'].groupby('arquivo')[r.metrica].mean().sort_index().values
        na=oldraw[oldraw.braco=='adaptativa'].groupby('arquivo')[r.metrica].mean().sort_index().values
        total=(na-nc).mean(); after=stat(x,y)
        after.update(efeito_nominal_absoluto=total,efeito_restante_pct_do_nominal=100*after['dif_absoluta']/total if total else np.nan)
        registrar('modelo_07_ablacao_mecanismo.csv','legado',dict(metrica=r.metrica,termo=r.configuracao),r._asdict(),after)
    num=pd.DataFrame(registros); keys=['fonte','metrica','termo','numero']
    control=num[num.modelo=='legado'][keys+['DEPOIS']].rename(columns={'DEPOIS':'CONTROLE_mesmo_ambiente'})
    num=num.merge(control,on=keys,validate='many_to_one')
    num['delta_vs_controle']=num.DEPOIS-num.CONTROLE_mesmo_ambiente
    num.to_csv(out/'antes_depois_numeros_B9_fatorial.csv',index=False)
